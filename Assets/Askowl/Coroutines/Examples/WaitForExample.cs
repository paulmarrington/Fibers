...
namespace Askowl.Fibers {
  public class WaitForExample {
    ...
    public IEnumerator WaitForUpdates() {
      counter  = 0;
      recycled = 0;
      // Sample - can just user Fiber(ExampleSimpleFiber) in MonoBehaviour
    ...
    private void CheckFiber(Func<IEnumerator> fiber) {
      ...
      Assert.AreEqual(fiberWorker.Recycled.Count, recycled + 1, "Expecting fiber to be back in Recycled");
    }
...
//      CheckFiber(ExampleSimpleActionFiber);
    ...
    private IEnumerator Updates(Func<IEnumerator> coroutine, float secondsDelay) {
      ...
      Assert.IsTrue(this.coroutine.Yield.EndYieldCondition(this.coroutine), "Fiber did not complete");
    }
  }
}