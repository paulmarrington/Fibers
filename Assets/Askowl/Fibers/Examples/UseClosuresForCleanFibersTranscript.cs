...
//- It turns out that precompiling fibers is not enough. A fiber will, but it's very nature, survive long after the code that called it has moved on. In OO a class instance is the closure. Fortunately, C# 7 has syntax that can make it concise with a minimum of visible scaffolding. This example is production code taken from the CustomAsset package for calling external asynchronous services. If you use the the following pattern you can use fibers any time and anywhere without worrying about the typical multi-tasking concerns.
using System;
...
namespace Askowl.Fibers.Transcripts {
  public class UseClosuresForCleanFibersTranscript {
    ...
    [UnityTest] public IEnumerator FiberClosureTest() {
      ...
      //- If we need the results longer we will need to take a copy of the scope. Because a tuple is a struct, the assignment is a copy, not a reference.
  ...
  //- I'll finish up with a real-world example, being the Service subsystem of the CustomAsset package.
  public class Services<TS, TC> {
    ...
    //- The above is just placeholders for code in the service class. CallService is the only public method needed to call the service.
